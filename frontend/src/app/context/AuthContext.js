    ...
    const initAuth = async () => {
      try {
        ...
        if (result.authenticated && result.user) {
          ...
          // Show token expired notification if this was due to an auth error
          if (result.tokenExpired) {
            notify.error("Your token has expired. Please sign in again.", 5000);
          }
        }
      } catch (error) {
